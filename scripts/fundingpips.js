/* ThePropFirmGuide • FundingPips (Full Page) — Single-file JS build
   Drop into GitHub, then load via jsDelivr in a Wix HTML Embed.
   Author: you + your AI pair 🛠️
*/
...
(function(){
  ...
  style.textContent = `
  :root{--bg:#0b0b0b;--card:#111;--text:#fff;--muted:#b9c1c9;--cyan:#33ccff;--cyan-20:rgba(51,204,255,.2);--border:#1f2937;--shadow:0 12px 30px rgba(0,0,0,.35);--radius:18px;--font:'Montserrat',sans-serif;--success:#31d0aa;--danger:#ff4d6d;--star:#ffd166;--grad:linear-gradient(90deg, rgba(51,204,255,.20), rgba(49,208,170,.20));}
  *{box-sizing:border-box} body{background:transparent;color:var(--text);font-family:var(--font);line-height:1.8;margin:0}
  .wrap{max-width:980px;margin:0 auto;padding:18px}
  .glass{background:var(--card);border:1px solid var(--border);border-radius:var(--radius);box-shadow:var(--shadow);position:relative;transition:transform .45s ease,box-shadow .45s ease,border-color .3s ease}
  .glass:hover{transform:translateY(-3px);box-shadow:0 16px 34px rgba(51,204,255,.25);border-color:#1e3a4f}
  .topbar{position:sticky;top:0;z-index:20;backdrop-filter:saturate(1.2) blur(6px);background:linear-gradient(180deg,rgba(0,0,0,.85),rgba(0,0,0,.65));border-bottom:1px solid #0f172a}
  ...
  .pill{display:inline-flex;align-items:center;gap:8px;padding:8px 12px;border-radius:999px;background:var(--grad);border:1px solid var(--cyan);color:#cfefff;font-weight:900;font-size:.8rem}
  ...
  .btn{display:inline-block;background:var(--cyan);color:#000!important;padding:12px 18px;border-radius:12px;font-weight:900;text-transform:uppercase;letter-spacing:.05em;box-shadow:0 0 18px var(--cyan-20);border:2px solid transparent;text-decoration:none;transition:.25s ease}
  ...
  .badge{background:linear-gradient(180deg,rgba(51,204,255,.16),rgba(51,204,255,.06));border:1px solid var(--cyan);color:#cfefff;padding:6px 10px;border-radius:999px;font-weight:900;font-size:.8rem;display:inline-flex;align-items:center;gap:6px}
  ...
  .tab-btn{background:#0f0f10;color:#fff;border:1px solid transparent;padding:10px 14px;border-radius:10px;font-weight:900;cursor:pointer;transition:.2s ease;display:inline-flex;align-items:center;gap:8px}
  ...
  .table-wrap{font-weight:700;width:100%;margin-top:8px}
  ...
  .input,.select{width:100%;padding:8px 12px;font-size:14px;border:1px solid var(--cyan);border-radius:8px;background:#111;color:#fff}
  .challenges-table{width:100%;border-collapse:collapse;text-align:center;font-size:13px;border-radius:12px;overflow:hidden}
  .challenges-table thead th{background:#1a1a1a;border-bottom:3px solid var(--cyan);text-transform:uppercase;letter-spacing:.05em;padding:12px 8px;position:sticky;top:0;z-index:2}
  .challenges-table td{padding:12px 8px;border-bottom:1px solid #222;vertical-align:middle}
  ...
  .sort-icons{display:inline-block;margin-left:4px;vertical-align:middle}
  .sort-icons .up,.sort-icons .down{display:block;width:0;height:0;border-left:4px solid transparent;border-right:4px solid transparent;margin:2px auto}
  .sort-icons .up{border-bottom:6px solid #fff}.sort-icons .down{border-top:6px solid #fff}
  ...
  .splitbar .track{position:relative;width:100%;height:10px;background:#0f172a;border:1px solid var(--border);border-radius:999px;overflow:hidden;box-shadow:inset 0 1px 0 rgba(255,255,255,.06)}
  .splitbar .fill{height:100%;width:0%;background:linear-gradient(90deg, rgba(51,204,255,0.85), rgba(49,208,170,0.85));box-shadow:0 0 10px rgba(51,204,255,.35);transition:width .35s ease}
  ...
  .stars::after{content:"★★★★★";position:absolute;left:0;top:0;width:calc(var(--rating)/5*100%);overflow:hidden;color:var(--star);letter-spacing:2px}
  ...
  @media (max-width:820px){.hero{grid-template-columns:1fr;text-align:center}.logo-lg{margin:0 auto}.stats{grid-template-columns:repeat(2,1fr)}.scores{grid-template-columns:repeat(2,1fr)}.columns{grid-template-columns:1fr}}
  @media (max-width:520px){.stats{grid-template-columns:1fr}.scores{grid-template-columns:1fr}}
  `;
  ...
  <header class="topbar">
    <nav class="nav wrap">
      <div class="brand">
        <div class="brand-logo"><img id="brandImg" alt="FundingPips logo" style="width:100%;height:100%;object-fit:cover"/></div>
  ...
  <main class="wrap">
    <section class="hero reveal on">
      <div class="logo-lg glass"><img id="heroLogo" alt="FundingPips logo" style="width:100%;height:100%;object-fit:cover"/></div>
          ...
          <table id="propTable" class="challenges-table glass">
            <thead>
              <tr>
                ...
                <th class="sortable" data-col="1">Program <span class="sort-icons"><span class="up"></span><span class="down"></span></span></th>
                <th class="sortable" data-col="2">Profit Target <span class="sort-icons"><span class="up"></span><span class="down"></span></span></th>
                <th class="sortable" data-col="3">Max Daily Loss <span class="sort-icons"><span class="up"></span><span class="down"></span></span></th>
                <th class="sortable" data-col="4">Max Total Loss <span class="sort-icons"><span class="up"></span><span class="down"></span></span></th>
                ...
                <th class="sortable" data-col="7">Fee <span class="sort-icons"><span class="up"></span><span class="down"></span></span></th>
                <th>Website</th>
              </tr>
            </thead>
  ...
  const rowHtml = r => {
    ...
    return `
      <tr>
        ...
        <td>${r.fee}</td>
        <td><a href="${r.url}" target="_blank" rel="noopener"><button class="visit-btn">Visit</button></a></td>
      </tr>`;
  };
  ...
  Array.from(new Set(DATA.challenges.map(r=>r.size))).sort((a,b)=>parseInt(a)-parseInt(b)||a.localeCompare(b))
  ...
  table.querySelectorAll("thead th.sortable").forEach(th=>{
    th.addEventListener("click", ()=>{
      ...
      const rows = [...DATA.challenges].map(row => {
        ...
        const keyMap = [
          row.size, row.program, row.target, row.daily, row.total, split, row.payoutFreq||"", row.fee
        ];
        return { row, keyMap };
      });
      ...
      rows.sort((A,B)=>{
        ...
        const numericCols = new Set([0,5,7]); // size, split, fee
